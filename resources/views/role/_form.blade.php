            ...
            <div class="form-group">
                    @foreach($permissions as $a)
                        <dl class="permission_list">
                            <dt>
                                <label class="checkbox-inline"><input {{ in_array($a['id'], $perms) ? 'checked' : '' }} type="checkbox" name="permission[]" value="{{ $a->id }}" > <strong>{{ $a->display_name }}</strong></label>
                            </dt>
                            <dd>
                                @foreach($a['child'] as $b)
                                    <dl class="cl permission_list2">

                                        <dt><label class="checkbox-inline"><input {{ in_array($b['id'], $perms) ? 'checked' : '' }} type="checkbox" name="permission[]" value="{{ $b->id }}" > <strong>{{ $b->display_name }}</strong></label></dt>
                                        <dd>
                                            @foreach($b['child'] as $c)
                                                <label class="checkbox-inline"><input {{ in_array($c['id'], $perms) ? 'checked' : '' }} type="checkbox" name="permission[]" value="{{ $c->id }}" > {{ $c->display_name }}</label>
                                            @endforeach
                                        </dd>
                                    </dl>
                                @endforeach
